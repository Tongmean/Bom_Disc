...
const getFiltermaterialfordrawing = async (req, res) => {
    ...
    const sqlCommand = `
        SELECT 
            ...
            -- WD Types (WD1-WD4)
            COALESCE(MAX(CASE WHEN ("Type2" = 'WD' OR "Type2" = 'FP') AND ("Type3" = 'WD-1' OR "Type3" = 'WD' OR "Type3" = 'FC') THEN "ID" END), '-') AS "Id_WD1",
            COALESCE(MAX(CASE WHEN ("Type2" = 'WD' OR "Type2" = 'FP') AND ("Type3" = 'WD-1' OR "Type3" = 'WD' OR "Type3" = 'FC') THEN "Quantity_Shim" END), '-') AS "Quantity_WD1",
            COALESCE(MAX(CASE WHEN ("Type2" = 'WD' OR "Type2" = 'FP') AND "Type3" = 'WD-2' THEN "ID" END), '-') AS "Id_WD2",
            COALESCE(MAX(CASE WHEN ("Type2" = 'WD' OR "Type2" = 'FP') AND "Type3" = 'WD-2' THEN "Quantity_Shim" END), '-') AS "Quantity_WD2",
            COALESCE(MAX(CASE WHEN ("Type2" = 'WD' OR "Type2" = 'FP') AND "Type3" = 'WD-3' THEN "ID" END), '-') AS "Id_WD3",
            COALESCE(MAX(CASE WHEN ("Type2" = 'WD' OR "Type2" = 'FP') AND "Type3" = 'WD-3' THEN "Quantity_Shim" END), '-') AS "Quantity_WD3",
            COALESCE(MAX(CASE WHEN ("Type2" = 'WD' OR "Type2" = 'FP') AND "Type3" = 'WD-4' THEN "ID" END), '-') AS "Id_WD4",
...
const getFiltermaterialbycompactnoshim = async (req, res) => {
    ...
    try {
        dbconnect.query(sqlCommand, [Compact_No_Modify_Drawing], (err, result) => {
            console.log("Compact_No_Modify_Drawing", Compact_No_Modify_Drawing)
