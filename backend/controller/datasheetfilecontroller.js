...
const getdatasheetfiles = async (req, res) => {
    try {
        dbconnect.query(`SELECT * FROM "datasheetfile"`, (err, result) => {
            if (err) {
                res.status(500).json({
                    success: false,
...
const getDatasheetfile = async (req, res) => {
    const id = req.params.id;
    try {
        dbconnect.query(`SELECT * FROM "datasheetfile" WHERE id = $1`,[id] , (err, result) => {
            if (err) {
                res.status(500).json({
                    success: false,
                    msg: "ดึงข้อมูลไม่สำเร็จ",
...
const uploadDir = path.join(__dirname, '../Assets/Datasheet');
...
// const uploadDatasheetMiddleware = upload.single('file');
const uploadDatasheetMiddleware = (req, res, next) => {
    upload(req, res, (err) => {
        if (err) {
...
const createDatasheetfile = async (req, res) => {
    const { datasheet_no } = req.body;
    ...
    //relative path
    const relativeFilePath = `Datasheet/${filename}`;
    const sqlCheck = `SELECT * FROM "datasheetfile" WHERE "datasheet_no" = $1`;
    const checkResult = await dbconnect.query(sqlCheck, [datasheet_no]);
    //Check if file exist
    // Fetch the old file details
    const fetchOldFileQuery = `SELECT path, unqiuename FROM "datasheetfile" WHERE unqiuename = $1`;
    ...
    if (checkResult.rows.length > 0) {
        ...
        return res.status(400).json({
            ...
            msg: `รหัส: ${datasheet_no} มีไฟล์อยู่แล้ว กรุณาลองรหัสใหม่!`
        });
    }

    try {
        const sqlCommand = `
            INSERT INTO "datasheetfile" 
            (datasheet_no, unqiuename, originalname, path, create_by) 
            VALUES ($1, $2, $3, $4, $5) 
            RETURNING *`;
        const values = [datasheet_no, filename, encodedOriginalName, relativeFilePath, userEmail];

        const result = await dbconnect.query(sqlCommand, values);
        res.status(200).json({ msg: `บันทึกไฟล์ : ${datasheet_no} สำเร็จแล้ว `, data: result.rows[0] , success: true});
    } catch (error) {
        console.error('Database error:', error.message);
        res.status(500).json({ msg: `บันทึกไฟล์ : ${datasheet_no} ไม่สำเร็จแล้ว กรุณาตรวจสอบอีกรอบ !!` , success: false});
    }
};
...
const updateShimFile = async (req, res) => {
    ...
    try {
        // Fetch the old file details
        const fetchOldFileQuery = `SELECT path, unqiuename FROM "datasheetfile" WHERE id = $1`;
        ...
        const currentValueResult = await dbconnect.query(`SELECT * FROM "datasheetfile" WHERE id = $1`, [id]);
        ...
        const updateQuery = `
            ...
            SET datasheet_no = $1, unqiuename = $2, originalname = $3, path = $4, create_by = $5 
            WHERE id = $6 
            RETURNING *`;
        const values = [datasheet_no, filename, encodedOriginalName, relativeFilePath, userEmail, id];
        ...
        for (const column of columns) {
            ...
            if (oldValue !== newValue) {
                await logUpdate("datasheetfile", column, id, oldValue, newValue, "updated" , userEmail);
            }
        }

        res.status(200).json({
            msg: `อัปเดตไฟล์ Shim: ${datasheet_no} สำเร็จแล้ว`,
        ...
        res.status(500).json({
            msg: `อัปเดตไฟล์ Shim: ${datasheet_no} ไม่สำเร็จ กรุณาตรวจสอบอีกรอบ !!`,
            success: false
        });
    }
};
