...
import { updateD_Pressureapi,fetchD_Pressure } from '../../Ultility/D_Pressure';
...
const UpdateD_Pressure = () => {
    ...
    useEffect(() => {
        const fetchsellectD = async () => {
            try {
                ...
                const data = (await fetchD_Pressure(id)).data[0]; // Fetch data by ID
                if (data) {
                    form.setFieldsValue(data); 
                    console.log('data', data)
                    console.log('Data_Sheet_No', data.Data_Sheet_No)
                    if (data) {
                        handleholecoldChange(data.Mold_Machine_Cold_Code);
                      }
                    
                    if (data) {
                        handleholehotChange(data.Mold_Machine_Cold_Hot);
                    }
                    if (data) {
                        handledweightChange(data.Data_Sheet_No);
                    }
                    // if (data) {
                    //     handlechangePcs_Per_Mold_Cold(data.Pcs_Per_Mold_Cold);
                    // }
                    // if (data) {
                    //     handlechangePcs_Per_Mold_Hot(data.Pcs_Per_Mold_Hot);
                    // }
                }
    ...
    //   try {
    //     const result = await fetchsellectbyD_moldmachine(moldmachine)
    ...
    //   try {
    //     const result = await fetchsellectbyD_Weights(dweight)
    //     console.log('resulthandledweightChange', result)
    //     if(result){
    ...
    //         console.log('Area', Area);
    //         console.log('Pressure_Cold', Pressure_Cold);
    //         console.log('Pressure_Hot', Pressure_Hot);
    ...
    //   } catch (error) {
    //     console.log('error',error)
    //     showNotification('Failed to fetch Dweight', 'warning');
    //   }
    // }
    ...
    return (
        <div className="container-fluid">
            <h2>แบบฟอร์มแก้ไข้ข้อมูล Data-Sheet (แรงดัน)</h2>
            <Form
...
export default UpdateD_Pressure;
