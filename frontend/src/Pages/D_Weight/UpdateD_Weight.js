...
import { updateD_Weightapi, fetchD_Weight } from '../../Ultility/D_Weight';
...
const UpdateD_Weight = () => {
    ...
    useEffect(() => {
        const fetchsellectD = async () => {
            try {
                ...
                const data = (await fetchD_Weight(id)).data[0]; // Fetch data by ID
                if (data) {
                    form.setFieldsValue(data); 
                    if (data.Compact_No) {
                        handlecompactnoChange(data.Compact_No);
                      }
                    
                    if (data.Chem_Grade) {
                        handlechemcodeChange(data.Chem_Grade);
                    }
    ...
    const handleSubmit = async (values) => {
        ...
        try {
            const result = await updateD_Weightapi(id, Data);
            ...
            setTimeout(() => navigate('/dweight'), 2500);
        } catch (error) {
    ...
    return (
        <div className="container-fluid">
            <h2>แบบฟอร์มแก้ไข Data-Sheet (น้ำหนัก)</h2>
            <Form
